...
in [`../deploy/qemu/`](../deploy/qemu/README.md)) so extension **101** works, **without
touching the human-first 111 line**. Follows [SOP 19 §8–§10](../SOP/19-AI-101-Design.md).
...
- Add `ctx_ai_101`, `ctx_ai_196`, `ctx_ai_fallback` from
  ...
  [`../config/extensions_custom.conf`](../config/extensions_custom.conf). **Test order:**
...
- **ARI creds:** strong, random, **used only over localhost/mgmt**; rotate the VM's dev
  defaults per the [qemu hardening checklist](../deploy/qemu/README.md#production-hardening-checklist).
...
Per [SOP 19 §9](../SOP/19-AI-101-Design.md) — **separate from the 111 health check**. 101
...
🔒 Follows [SOP 19 §10](../SOP/19-AI-101-Design.md). **Do not skip stages.**
...
- 🔒 Secrets never committed; recordings/logs retention stays under
  [SOP 12 §7](../SOP/12-Incident-Logging-Schema.md).
