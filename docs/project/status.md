...
| Asterisk config + helper scripts + provisioning CSVs | ✅ Done | In [`../config/`](../config/), [`../scripts/`](../scripts/), [`../provisioning/`](../provisioning/) |
| QEMU Ubuntu VM PBX (autostart, LAN-facing, one-command deploy) | ✅ Done | [`../deploy/qemu/`](../deploy/qemu/) |
| Operations Console (live status + client generator) | ✅ Done, localhost only | [`../Console/`](../Console/); LAN serve not yet enabled |
...
| Recorded voice prompts | ❌ Not done — buildable | Currently placeholder/TTS; wording ready in [SOP 28](../SOP/28-Voice-Prompt-Scripts.md) |
| QR/profile phone provisioning | 🔄 In progress — buildable | [`../provisioning/linphone/`](../provisioning/) being created |
...
| Test-evidence sheet filled | ❌ Not done — buildable | [SOP 32](../SOP/32-Test-Evidence-Sheet.md) template ready |
...
| Always-on ERT answer-point Androids | ❌ Not done — needs hardware | Battery/background hardening per [SOP 24](../SOP/24-Mobile-App-Reliability-and-Battery.md) |
| UPS / van power | ❌ Not done — needs hardware | Risk R1, [SOP 21](../SOP/21-Risk-Register-and-Gaps.md) |
| mDNS / DHCP reservation (no reconfig across networks) | ❌ Not done — needs decision | |
| Full pilot + go-live sign-off | ❌ Not done — needs execution | [SOP 17](../SOP/17-Pilot-Test-Plan.md), [SOP 18](../SOP/18-Go-Live-Checklist.md) |
...
|---|---|
| Full documentation set | 32 SOP docs ([`../SOP/`](../SOP/)), 8 Blueprint docs ([`../Blueprint/`](../Blueprint/)), Journal logs: [Roadblocks-and-Solutions.md](./Roadblocks-and-Solutions.md), [Doc-Fixes.md](./Doc-Fixes.md), [Field-Test-Issues-and-Mitigations.md](./Field-Test-Issues-and-Mitigations.md) |
| Working Asterisk config, helper scripts, provisioning CSVs | [`../config/`](../config/), [`../scripts/`](../scripts/), [`../provisioning/`](../provisioning/) |
| Running QEMU Ubuntu VM PBX | [`../deploy/qemu/`](../deploy/qemu/) — autostarts on Windows logon; Asterisk autostarts inside the VM; LAN-facing (SIP 5060 + RTP 10000–10019 on 192.168.1.16) |
| Auto-rebind to current network on boot | [`Set-UpesLanIp.ps1`](../deploy/qemu/Set-UpesLanIp.ps1) |
| One-command deploy | [`Deploy-UpesEcsVm.ps1`](../deploy/qemu/Deploy-UpesEcsVm.ps1) |
| Operations Console web page | [`../Console/`](../Console/) — live status + "register a client" generator |
...
| Four real SAP-ID accounts provisioned | See [`../Notes/Confirmed Details.md`](../Notes/Confirmed%20Details.md) and [`../provisioning/pilot-users.csv`](../provisioning/pilot-users.csv) |

**Real roster provisioned** (from [`../Notes/Confirmed Details.md`](../Notes/Confirmed%20Details.md) — the only source of real IDs):
...
|---|---|
| `direct_media=no` | [`../deploy/asterisk/pjsip.conf`](../deploy/asterisk/pjsip.conf) |
| `+91` strip in dialplan | [`../config/extensions_custom.conf`](../config/extensions_custom.conf) |
| Four real users added | [`../Notes/Confirmed Details.md`](../Notes/Confirmed%20Details.md) and [`../provisioning/pilot-users.csv`](../provisioning/pilot-users.csv) |
...
| Cost / Bill of Materials | Skipped (user decision) — note [Blueprint 05](../Blueprint/05-Bill-of-Materials.md) and [SOP references] remain as reference only |
...
|---|---|---|---|
| B1 | **Real recorded voice prompts** | Currently placeholder/TTS. Real prompts make the system sound production-grade and unambiguous in an emergency. Exact wording is already written. | [SOP 28](../SOP/28-Voice-Prompt-Scripts.md) |
| B2 | **QR / profile phone provisioning** | Eliminates the transport / `+91` / encryption misconfiguration that phones hit today when set up by hand. One scan → correct config. | [`../provisioning/linphone/`](../provisioning/) (being created) |
| B3 | ~~**ERT answer point at position `4110`**~~ ✅ **Done** | The ERT queue is now staffed by the on-shift registered handset (`ert-shift.sh` / shift codes `*22`/`*23`), so a `111` call **rings a real phone and is answerable** (wallboard READY). When nobody answers, the call routes to the offline panic-coach (`102`). | [SOP 02](../SOP/02-ERT-SOP.md), [`../provisioning/responder-positions.csv`](../provisioning/responder-positions.csv) |
| B4 | **Filled test-evidence sheet** | Converts "it worked when we tried it" into a documented, repeatable evidence record. | [SOP 32](../SOP/32-Test-Evidence-Sheet.md) |
| B5 | **Console served over LAN** | Console is localhost-only today. LAN serve needs an elevated bind + a TCP 8080 firewall rule. | [`../Console/`](../Console/) |
...
|---|---|---|---|
| H1 | **Move production PBX off QEMU SLIRP NAT** — to the van's own Linux box, or bridged networking with a real LAN IP | **The #1 production step.** SLIRP NAT is the root cause of the whole media/NAT class of issues; the `direct_media`/`rtp_symmetric` fixes are workarounds for it. A real LAN IP removes the entire problem category. | [Blueprint 04](../Blueprint/04-Network-and-Deployment.md), [SOP 23](../SOP/23-Mobile-Van-Deployment.md) |
| H2 | **TLS + SRTP** for encrypted signalling and media | Currently plain UDP/RTP — calls are unencrypted on the LAN. Requires cert setup and a client-config decision. | [SOP 26](../SOP/26-Security-Hardening.md) |
| H3 | **Dedicated always-on ERT answer-point Androids** | Requires physical phones with battery/background hardening so the SIP app never sleeps and always rings. | [SOP 24](../SOP/24-Mobile-App-Reliability-and-Battery.md) |
| H4 | **UPS / power for the van** | Physical power hardware; without it the PBX dies on any power interruption. Tracked as Risk **R1**. | [SOP 21](../SOP/21-Risk-Register-and-Gaps.md) |
| H5 | **mDNS / DHCP reservation** so phones need no reconfig across networks | Requires network infrastructure config so the PBX has a stable name/address regardless of which LAN the van joins. | [Blueprint 04](../Blueprint/04-Network-and-Deployment.md) |
| H6 | **Full pilot + go-live sign-off** | Requires executing the structured pilot and obtaining formal sign-off — an activity, not a build. | [SOP 17](../SOP/17-Pilot-Test-Plan.md), [SOP 18](../SOP/18-Go-Live-Checklist.md) |
...
5. **B1 — record the real voice prompts** ([SOP 28](../SOP/28-Voice-Prompt-Scripts.md)).
6. **B5 — serve the Console over the LAN** so operators can use it from the van, not just localhost.
7. **B4 — fill the test-evidence sheet** ([SOP 32](../SOP/32-Test-Evidence-Sheet.md)) to lock in repeatable proof.
...
10. **H6 — run the full pilot ([SOP 17](../SOP/17-Pilot-Test-Plan.md)) and complete go-live sign-off ([SOP 18](../SOP/18-Go-Live-Checklist.md)).**

---

### Related documents
- Journal: [Roadblocks-and-Solutions.md](./Roadblocks-and-Solutions.md) · [Doc-Fixes.md](./Doc-Fixes.md) · [Field-Test-Issues-and-Mitigations.md](./Field-Test-Issues-and-Mitigations.md)
- [SOP index](../SOP/00-README.md) · [Blueprint index](../Blueprint/00-README.md)
- Deploy: [`../deploy/qemu/README.md`](../deploy/qemu/README.md) · Console: [`../Console/README.md`](../Console/README.md)
